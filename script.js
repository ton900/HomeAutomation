...
"aoColumnDefs": [
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", "default.css");
            $.cookie('skin_color', color);
			$("#content").css("background-color",$("#sidebar").css("background-color"));
        }
	...
	{
		$(document).delegate(".aClass","click",function(e){	
			var notify_type = $(this).find('#hidNotifyType').val();
			var notify_id = $(this).find('#hidNotifyID').val();
			
			if(notify_type == 0)
			{
				$.ajax({ 
					url: "ajax_readNotify.php" ,
					type: "GET",
					data: { id: notify_id }
				});	
			}		
